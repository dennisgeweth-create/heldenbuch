...
function App() {
  ...
  useEffect(() => { kontoRef.current = konto; }, [konto]);
  useEffect(() => { besitzerRef.current = besitzer; }, [besitzer]);
  useEffect(() => { advDmsRef.current = advDms; }, [advDms]);
  ...
  useEffect(() => {
    ...
    try { localStorage.removeItem('sv_pass'); } catch {}
    ...
    if (token) {
      apiMe(url).then(d => setKonto(d.user || null)).catch(() => {
        try { localStorage.removeItem('sv_token'); } catch {}
        setKonto(null);
        setShowSetup(true);
      });
    } else {
      ...
      if (verbunden(zug)) {
        apiSaveLibrary(zug.url, zug.code, zug.pass, lib)
          ...
          .catch(e => appAlert('Die Datenbank konnte nicht gespeichert werden: '
            ...
            + ' — auf diesem Gerät steht sie damit anders da als auf dem Server. '
            + 'Mach eine Sicherung (⬇ unten im Datenbankfenster), bevor du neu lädst.'));
      }
      return lib;
    });
  };
  ...
  const applyKontoSetup = async () => {
    ...
    try {
      ...
      if (!gcode) {
        ...
        throw new Error(k && k.ist_admin
          ? 'Es gibt noch keine Gruppe. Leg unter 🛠 Verwaltung eine an.'
      ...
      setKonto(k);
      setSvUrl(url); setSvCode(gcode); setSvPass('');
      applyChars(data.chars || []);
      pendingRef.current = false;
      spiegleChars(JSON.stringify(data.chars || []));
      if (data.library) { setUserLibrary(data.library); setLibGeladen(true); safeSetItem('hb_library', JSON.stringify(data.library)); }
      ...
      if (k && k.muss_wechseln) setPasswortDlg({alt: pw, neu: '', neu2: '', err: '', pflicht: true});
    } catch(e) {
      localStorage.removeItem('sv_token');
  ...
  const gruppeAnlegen = async () => {
    ...
    if (!/^[A-Za-z0-9_\-]{3,20}$/.test(name)) {
      setVerwaltung(v => ({...v, err: 'Der Code braucht 3 bis 20 Zeichen: Buchstaben, Zahlen, - und _.'}));
      return;
    }
    try {
      ...
      setVerwaltung(v => ({...v, neueGruppe: '', err: '',
                           gezeigt: {name, pw: '—', was: 'als Gruppe angelegt'}}));
  ...
  const passwortAendern = async () => {
    const d = passwortDlg;
    if (!d) return;
    if ((d.neu || '').length < 6) { setPasswortDlg({...d, err: 'Mindestens 6 Zeichen.'}); return; }
    if (d.neu !== d.neu2)         { setPasswortDlg({...d, err: 'Die beiden Eingaben sind nicht gleich.'}); return; }
    try {
      await apiPassAendern(serverCreds().url, d.alt, d.neu);
      setKonto(k => k && ({...k, muss_wechseln: false}));
      setPasswortDlg(null);
    } catch(e) { setPasswortDlg({...d, err: e.message}); }
  };
  ...
  useEffect(() => {
    ...
    const frage = async () => {
      ...
      const takt = document.hidden ? 12000
        : kampfSichtRef.current ? TAKT_KAMPF : 12000;
  ...
  const beuteWegraeumen = async () => {
    const {url, code, pass} = serverCreds();
    try { await apiBeuteSetzen(url, code, pass, advId, null); setBeute(null); setBeuteOffen(false); beuteStandRef.current = -1; gleichFragen('beute'); }
    catch {}
  };
  ...
  const probeSetzen = async (p) => {
    ...
    try {
      ...
      if (d && d.probe) { probeRef.current = d.probe; setProbe(d.probe); }
  ...
  const probeAbraeumen = async () => {
    ...
    try {
      ...
      probeRef.current = null; setProbe(null); probeStandRef.current = -1;
      gleichFragen('probe');
    } catch {}
  };
  ...
  const probeAntworten = async (c, wurf, bonus) => {
    ...
    const sofort = {charId: c.id, name: c.name, wurf, bonus, zeit: Math.floor(Date.now()/1000)};
    setProbe(p => {
      if (!p) return p;
      const rest = (p.antworten || []).filter(a => a.charId !== c.id);
      const neu = {...p, antworten: [...rest, sofort]};
    ...
    try {
      ...
      if (d && d.ansage) {
        setKampfSichtDaten(k => {
          ...
          const neu = drin ? k : {...k, ansagen: [...(k.ansagen || []), d.ansage]};
          kampfSichtRef.current = neu;
          return neu;
        });
      }
        ...
        <div className={"sidebar"+(sidebarCollapsed?" collapsed":"")}>
          <div className="sidebar-header">
            ...
            <div className="sidebar-wort">Heldenbuch
              <button className="app-version" onClick={()=>{ setPatchnotesOffen(true); leiste.zeigen('patchnotes'); }}
          ...
          <div className="sidebar-footer">
            ...
            {svCode ? (
              ...
              <button className="btn-sync" onClick={()=>{setSetupErr('');
                setSetupForm(f=>({...f, pass:''})); setShowSetup(true);}}>👤 Anmelden</button>
            )}
          </div>
        </div>
          ...
          <div className="form-modal" style={{maxWidth:620}} onClick={e=>e.stopPropagation()}>
            ...
            {verwaltung.gezeigt && (
              <div className="verw-passwort">
                <div>
                  <b>{verwaltung.gezeigt.name}</b> {verwaltung.gezeigt.was}.
                  {verwaltung.gezeigt.pw !== '—' && <>
                    {' '}Einmalpasswort:
                ...
                <i>
                  {verwaltung.gezeigt.pw !== '—'
                    ? 'Gib es weiter — es steht nur hier und nur jetzt. Beim ersten Anmelden muss ein eigenes gewählt werden.'
                    : 'Wer dazugehört, bestimmst du oben über die Rolle. Ohne Mitglied ist eine Gruppe leer.'}
                </i>
            ...
            <div className="einst-roll">
              ...
              <EinstBlock titel="🗺 Neue Gruppe" kurz="anlegen">
                ...
                <div className="einst-klasse">
                  <input className="form-input" placeholder="Code, z.B. STRAHD" maxLength={20}
                    value={verwaltung.neueGruppe || ''}
                    onChange={e=>setVerwaltung(v=>({...v, neueGruppe:e.target.value.toUpperCase(), err:''}))}
        ...
        <Fenster>
          <div className="form-modal" style={{maxWidth:400}}>
            ...
            <div style={{fontSize:13,color:'var(--text-muted)',marginBottom:16,lineHeight:1.55}}>
              {passwortDlg.pflicht
                ? 'Du bist mit einem Einmalpasswort angemeldet. Wähle jetzt dein eigenes — das alte gilt danach nicht mehr.'
                : 'Das neue Passwort gilt sofort. Andere Geräte, auf denen du angemeldet bist, werden abgemeldet.'}
            </div>
            {!passwortDlg.pflicht && (
              <div className="form-group">
                <div className="form-label">Bisheriges Passwort</div>
                <input className="form-input" type="password" value={passwortDlg.alt}
                  onChange={e=>setPasswortDlg({...passwortDlg, alt:e.target.value, err:''})} />
              </div>
            )}
            <div className="form-group">
              <div className="form-label">Neues Passwort</div>
              <input className="form-input" type="password" autoFocus placeholder="Mind. 6 Zeichen"
                value={passwortDlg.neu}
                onChange={e=>setPasswortDlg({...passwortDlg, neu:e.target.value, err:''})} />
            </div>
            <div className="form-group">
              <div className="form-label">Noch einmal</div>
              <input className="form-input" type="password" value={passwortDlg.neu2}
                onChange={e=>setPasswortDlg({...passwortDlg, neu2:e.target.value, err:''})}
            ...
            <div className="form-actions">
              {!passwortDlg.pflicht && (
                <button className="btn-cancel" onClick={()=>setPasswortDlg(null)}>Abbrechen</button>
              )}
        ...
        <Fenster>
          <div className="form-modal" style={{maxWidth:420}}>
            ...
            <p style={{fontSize:13,color:"var(--text-muted)",lineHeight:1.6,marginBottom:16}}>
              Melde dich mit deinem Konto an. Deine Gruppe, deine Rolle und deine Helden
              hängen daran — mehr musst du nicht wissen.
            </p>
            <div className="form-grid" style={{gridTemplateColumns:"1fr"}}>
              <div className="form-group">
                <div className="form-label">Name</div>
                <input className="form-input" placeholder="Dein Kontoname" maxLength={40}
                  autoFocus value={setupForm.name||''}
            ...
            <div className="form-actions">
              ...
              <button className="btn-save" style={{flex:1,opacity:setupBusy?0.6:1}}
                onClick={applyKontoSetup} disabled={setupBusy}>
...
const AdventureLog = ({onClose, isDmMode, abenteuer}) => {
  ...
  return (
    <Fenster onClick={onClose} leiste={{id: 'abenteuerlog', titel: 'Abenteuerlog', symbol: '📖'}}>
